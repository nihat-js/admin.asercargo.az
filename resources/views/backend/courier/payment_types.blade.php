...
@section('actions')
    @if(Auth::id() == 1)
        <li>
            <a class="action-btn" onclick="del('{{route("admin_courier_payment_type_delete")}}')"><span
            ...
            <div class="panel-body">
                <div id="search-inputs-area" class="search-areas">
                    <input type="text" class="form-control search-input" id="search_values" column_name="name"
                           placeholder="Payment type" value="{{$search_arr['name']}}">
            ...
            <table class="references-table">
                ...
                @foreach($payment_types as $payment_type)
                    <tr class="rows" id="row_{{$payment_type->id}}" onclick="select_row({{$payment_type->id}})">
                        ...
                        <td id="name_{{$payment_type->id}}" name_en="{{$payment_type->name_en}}" name_az="{{$payment_type->name_az}}" name_ru="{{$payment_type->name_ru}}">{{$payment_type->name_en}}</td>
    ...
    <script>
        ...
        @if(Auth::id() == 1)
            function show_add_modal() {
                $('#form_item_id').html("");
                $(".add_or_update_form").prop("action", "{{route("admin_courier_payment_type_add")}}");
        ...
        function show_update_modal() {
            ...
            $(".add_or_update_form").prop("action", "{{route("admin_courier_payment_type_update")}}");
