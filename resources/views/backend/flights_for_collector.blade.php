...
@section('actions')
    @if(Auth::user()->role() != 11)
        <li>
            <a onclick="close_flight('{{route("close_flight_collector")}}');" class="action-btn"><span class="glyphicon glyphicon-time"></span> Close flight</a>
        </li>
    @endif
{{--    <li>--}}
...
{{--    </li>--}}
    ...
    <li>
        <a onclick="create_new_container_flight('{{route("add_container_collector")}}');" class="action-btn"><span class="glyphicon glyphicon-plus-sign"></span> Add Container</a>
    </li>

    <li>
        <a class="action-btn" onclick="del('{{route("delete_flight_collector")}}')"><span
        ...
        <div class="references-in">
            <table class="references-table">
                ...
                @foreach($flights as $flight)
                    <tr class="rows" id="row_{{$flight->id}}" onclick="select_row({{$flight->id}})" ondblclick="get_containers_by_flight('{{route("show_containers_collector") . "?search=1&flight=" . $flight->id}}')">
                        <td>{{$flight->id}}</td>
                        <td>{{$flight->name}}</td>
    ...
    <div class="modal fade" id="add-modal" tabindex="-1" role="dialog" data-backdrop="static"
        ...
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                ...
                <form id="form" class="add_or_update_form" action="{{route("add_flight_collector")}}" method="post">
                    {{csrf_field()}}
                        ...
                        <div class="form row">
                            <div class="col-md-6">
                                <p class="carrier">
                                    ...
                                    <input type="text" name="carrier" id="carrier" required="" maxlength="3" value="{{Auth::user()->location_airport()}}" readonly>
                                </p>
                            ...
                            <div class="col-md-6">
                                <p class="departure">
                                    ...
                                    <input type="text" name="departure" id="departure" required="" maxlength="50" value="{{Auth::user()->location_airport()}}">
                                </p>
                                <p class="destination">
...
{{--                                    <input size="16" type="text" value="2012-06-15 14:45" readonly class="form_datetime" name="plan_take_off" id="plan_take_off" required="">--}}
{{--                                </p>--}}
                            </div>
                        </div>
                    </div>
...
{{--                    </div>--}}
{{--                </div>--}}
{{--                <form id="form" action="{{route("set_fact_take_off_collector")}}" method="post">--}}
{{--                    {{csrf_field()}}--}}
...
{{--                    </div>--}}
{{--                </div>--}}
{{--                <form id="form" action="{{route("set_fact_arrival_collector")}}" method="post">--}}
{{--                    {{csrf_field()}}--}}
...
@section('js')
    ...
    <script>
        ...
        function show_add_modal() {
            $('#form_item_id').html("");
            $(".add_or_update_form").prop("action", "{{route("add_flight_collector")}}");
            $('.modal-title').html('Add flight');

            $("#carrier").val("{{Auth::user()->location_airport()}}");
            $("#flight_number").val("");
            $("#awb").val("");
            $("#departure").val("{{Auth::user()->location_airport()}}");
            $("#destination").val("GYD");
            //$("#plan_take_off").val("");
            // $("#plan_arrival").val("");
        ...
        function show_update_modal() {
            ...
            $(".add_or_update_form").prop("action", "{{route("update_flight_collector")}}");
        ...
        function create_new_container_flight(url) {
            var flight_id = row_id;
