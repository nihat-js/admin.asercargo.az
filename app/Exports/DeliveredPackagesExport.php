...
class DeliveredPackagesExport implements FromCollection, WithColumnFormatting, WithHeadings, ShouldAutoSize
{
    /**
    ...
    public function __construct($from_date, $to_date, $access_paid = 0)
    {
    ...
    public function collection()
    {
        try {
            ...
            $query = Item::leftJoin('package', 'item.package_id', '=', 'package.id')
                ...
                ->whereDate('package.delivered_at', '>=', $this->from_date)
                ->whereDate('package.delivered_at', '<=', $this->to_date)
                ->where('package.client_id', '<>', 121514); // sonra silinecek

            $packages = $query->orderBy('package.delivered_at')
                ->select(
            ...
            foreach ($packages as $package) {
                ...
                $warehouse_obj->created_date = substr($package->delivered_at, 0, 16);
